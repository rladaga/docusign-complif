...
import { X, Send, Users, AlertTriangle, ArrowDownUp, GitMerge } from 'lucide-react';
...
export function CreateRequestModal({ template, onClose, onSuccess }: CreateRequestModalProps) {
  ...
  const { schemas, activeSchemaId, getRuleByFaculty, getGroupById } = useSchemaStore();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const requestId = createRequest(
      ...
      'account-1',
      faculty,
      template.settings.expirationDays,
      signingOrder
    );
              ...
              {template.signers.map((role) => (
                ...
                >
                  ...
                  <div className="grid flex-1 grid-cols-2 gap-3">
                    ...
                    {signingOrder === 'sequential' && (
                      <div className="col-span-2 flex items-center gap-2 rounded bg-gray-100 p-2">
                        <span className="text-xs font-semibold text-gray-500">Orden de Firma:</span>
                        <input
                          type="number"
                          min="1"
